...
const STEP1_PROMPT = `あなたは理学療法士向けの医学辞典です。複数の教科書・ガイドラインをもとに回答してください。
マークダウン記号（**、##など）は使わず、プレーンテキストで回答してください。
以下の6項目について、各項目3点以内の箇条書きで簡潔に回答してください。
...
const STEP2_PROMPT = `あなたは理学療法士向けの医学辞典です。複数の教科書・ガイドラインをもとに回答してください。
以下の6つの各セクションについて、詳しい内容をマークダウン形式（**太字**・- 箇条書き・## 見出し）で追記し、
...
評価方法（ASSESSMENT）→ 理学療法評価学（渡邉好孝・医学書院）、標準理学療法学 評価編を含め3冊以上
治療方針（TREATMENT）→ 疾患区分別に3冊以上選ぶ：
