...
・順序性・因果関係のある内容（病態のメカニズム・治療の段階・治療手順など「一つの流れとして進行する」内容）には矢印（→）を使った流れ図形式で1行にまとめて表現してください。例：長期ステロイド投与 → 骨内圧上昇 → 微小循環障害 → 骨壊死。ただし、複数の独立した原因の列挙・単純な症状の列挙・評価項目の一覧など順序性がない内容には矢印を使わず箇条書き・表形式で表現してください。1つの回答内で必要な箇所にのみ矢印を使い、全体を矢印だらけにしないこと。
