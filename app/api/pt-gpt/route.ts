...
・回答の末尾に必ず「## 参考資料」セクションを追加し、使用した文献・教科書・ガイドラインを3冊以上リスト形式で明記すること${SUGGEST_RULES}`;
...
疾患・医療知識は以下の構成で回答：
## 定義・概要 ## 主な症状（3点以内） ## 評価・検査（3点以内） ## 予後予測・ゴール設定（3点以内） ## 治療方針・リハビリアプローチ（上記のアプローチ形式を使用） ## 注意事項・禁忌（3点以内） ## 臨床ポイント（3点以内） ## 参考資料（教科書・ガイドライン3冊以上）`;
