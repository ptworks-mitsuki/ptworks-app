    ...
    <div style={{ background: "#fff", borderRadius: 10, border: "1px solid #e5e7eb", padding: "16px 18px", marginBottom: 12, boxShadow: "0 1px 3px rgba(0,0,0,0.06)" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 8 }}>
        ...
        <button
          ...
          style={{ background: "none", border: "none", cursor: "pointer", fontSize: 22, padding: 0, color: isFavorited(favId) ? "#ef4444" : "#d1d5db", flexShrink: 0 }}
...
function PaperSearchTab({ initialQuery }: { initialQuery: string }) {
    ...
    <div>
      ...
      {results !== null && !analyzing && (
        <>
          ...
          {results.map(p => <PaperCard key={p.id} paper={p} />)}
          {results.length > 0 && (
...
function LiteratureSearchInner() {
  ...
  const [activeTab, setActiveTab] = useState<"paper" | "book">("paper");
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#f9fafb" }}>
      ...
      <div style={{ maxWidth: 720, margin: "0 auto", padding: "20px 16px" }}>
        ...
        {activeTab === "paper"
          ? <PaperSearchTab initialQuery={initQuery} />
          : <BookSearchTab />
        }
      </div>
    </div>
